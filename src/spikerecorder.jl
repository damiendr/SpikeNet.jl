...
struct RecordedSpikes{T,F<:Function,R,TT,TI}
    instance::T # the object we're recording from
    event::F    # event function
...
function RecordedSpikes(instance, event, steps)
    ...
    RecordedSpikes{typeof(instance), typeof(event), typeof(steps), TT, TI}(
                   instance, event, steps, ts, id)
end
...
@inline function record!(rec::RecordedSpikes, step)
    if step in rec.steps
        group = rec.instance
        ts = rec.ts
        id = rec.id
        for i in eachindex(group)
            @inbounds if rec.event(Elem(group,i))
                push!(ts, step)
                push!(id, i)
            end
        end
